...
GeodesicAlgorithmExact::GeodesicAlgorithmExact(
  SurfaceMesh &mesh_,
  IntrinsicGeometryInterface &geom_,
  EdgeData<IntervalList> &edge_interval_lists,
  VertexData<bool> &vertexIsManifold,
  VertexData<bool> &vertexIsBoundary
) : m_max_propagation_distance(1e100), mesh(mesh_), geom(geom_), m_memory_allocator(mesh_.nEdges(), mesh_.nEdges()){
  auto _start = std::chrono::high_resolution_clock::now();

  m_edge_interval_lists = edge_interval_lists;
  this->vertexIsManifold = vertexIsManifold;
  this->vertexIsBoundary = vertexIsBoundary;

  auto _end = std::chrono::high_resolution_clock::now();
  // std::cout << "initialize: " << std::chrono::duration_cast<std::chrono::microseconds>(_end - _start).count() << "us" << std::endl;
};
...
void GeodesicAlgorithmExact::propagate(const std::vector<SurfacePoint>& sources, double max_propagation_distance,
  ...
  // std::cout << "total " << itr << ", " << std::chrono::duration_cast<std::chrono::microseconds>(now - _start).count() << "us" << std::endl;
